...
//构造函数．
ICPMatcher::ICPMatcher(double _r,double _h,int _iter)
{
    m_r = _r;
    m_h = _h;
    m_Iterations = _iter;
...
void ICPMatcher::setIterations(int _iter)
{
    m_Iterations = _iter;
}

//去除非法数据
void ICPMatcher::RemoveNANandINFData(std::vector<Eigen::Vector2d> &_input)
{
    //去除非法数据．
...
void ICPMatcher::setSourcePointCloud(pcl::PointCloud<pcl::PointXYZ> &pcl_cloud)
{
...
void ICPMatcher::setSourcePointCloud(std::vector<Eigen::Vector2d> &_source_pcloud)
{
...
void ICPMatcher::setSourcePointCloudNormals(std::vector<Eigen::Vector2d> &_normals)
{
    m_sourcePtCloudNormals = _normals;
}

//设置目标点云．
void ICPMatcher::setTargetPointCloud(pcl::PointCloud<pcl::PointXYZ> &pcl_cloud)
{
...
//本函数会自动重新生成kd树．
void ICPMatcher::setTargetPointCloud(std::vector<Eigen::Vector2d> &_target_pcloud)
{
...
bool ICPMatcher::ImplicitMLSFunction(Eigen::Vector2d x,
                                         double& height)
{
...
/**
 * @brief ICPMatcher::projSourcePtToSurface
 ...
 */
void ICPMatcher::projSourcePtToSurface(
...
bool ICPMatcher::SolveMotionEstimationProblem(std::vector<Eigen::Vector2d> &source_cloud,
...
/**
 * @brief ICPMatcher::ComputeNormal
 * 计算法向量
 * @param nearPoints    某个点周围的所有激光点
 * @return
 */
Eigen::Vector2d ICPMatcher::ComputeNormal(std::vector<Eigen::Vector2d> &nearPoints)
{
    Eigen::Vector2d normal;
    //TODO
    double sum_pos_x = 0.0;
    double sum_pos_y = 0.0; 
    double average_pos_x = 0.0;
    double average_pos_y = 0.0;   

    for(int i=0; i< nearPoints.size(); ++i)
    {
       sum_pos_x += nearPoints[i][0];
       sum_pos_y += nearPoints[i][1];
    }
    average_pos_x = sum_pos_x / nearPoints.size();
    average_pos_y = sum_pos_y / nearPoints.size();

    Eigen::Matrix2d sigma;

    for(int i=0; i< nearPoints.size(); ++i)
    {
       Eigen::Vector2d delta_p(nearPoints[i][0] - average_pos_x, nearPoints[i][1] - average_pos_y);
       sigma += delta_p * delta_p.transpose();
    }
    ...
    Eigen::MatrixXd values_real=eigen_values.real();
    Eigen::MatrixXf::Index val_min;

    values_real.rowwise().sum().minCoeff(&val_min);

    //  std::cout<<"eigen_vectors.real()(0, val_min) "<<eigen_vectors.real()(0, val_min)<<std::endl; 
    //  std::cout<<" eigen_vectors.real()(1, val_min) "<< eigen_vectors.real()(1, val_min)<<std::endl; 
    normal(0) = eigen_vectors.real()(0, val_min);
    normal(1) = eigen_vectors.real()(1, val_min) ;
...
/**
 * @brief ICPMatcher::Match
 * 最终使用的ICP匹配函数．
 * @param finalResult
 * @param covariance
 * @return
 */
bool ICPMatcher::Match(Eigen::Matrix3d& finalResult,
                           Eigen::Matrix3d& covariance)
{
    ...
    if(m_isGetNormals == false)
    {
        ...
        for(int index_t = 0; index_t < m_targetPointCloud.size(); ++index_t)
        {  
            Eigen::Vector2d normal={std::numeric_limits<double>::infinity(),std::numeric_limits<double>::infinity()};
            // 找到位于点m_targetPointCloud 电云附近(m_r)的所有的点云
            int searchNumber = 20;
            Eigen::VectorXi nearIndices(searchNumber);
            Eigen::VectorXd nearDist2(searchNumber);
            ...
            //下标储存在nearIndices中，距离储存在nearDist2中．
            //最大搜索距离为m_r
            m_pTargetKDTree->knn(m_targetPointCloud[index_t],nearIndices,nearDist2,searchNumber,0,
                                Nabo::NNSearchD::SORT_RESULTS | Nabo::NNSearchD::ALLOW_SELF_MATCH|
                                Nabo::NNSearchD::TOUCH_STATISTICS,
                                0.15);
            ...
            for(int i = 0; i < searchNumber;i++)
            {
                        ...
                        std::isnan(nearDist2(i)) == false)
                {
                    ...
                    nearPoints.push_back(m_targetKDTreeDataBase.col(index));
                }
                else
                {
                    break;
...
bool ICPMatcher::SolverFourthOrderPolynomial(Eigen::VectorXd&p_coffi,
                                                 double &lambda)
{
